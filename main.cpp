...
// PRESS 'n'FOR NIGHT AND PRESS 'd' FOR DAY
...
void Moon()
{
    glPushMatrix(); //when night mode moon show
    glColor3f(1.0,1.0,1.0);
    circle(0.75,0.75,0.18);
    glPopMatrix(); //when day mode moon disappear
}
void Cloud1()
{
...
void Tree1()
{
    ...
    glBegin(GL_POLYGON); //tree bottom part with GACH-DAALA
    ...
    glBegin(GL_POLYGON); //1st part PATA
    glVertex2f(-0.36,0.04);
    glVertex2f(-0.26,0.28);
    glVertex2f(-0.16,0.04);
    glEnd();
    glBegin(GL_POLYGON); //2nd part PATA
    glVertex2f(-0.36,-0.04);
    glVertex2f(-0.26,0.20);
    glVertex2f(-0.16,-0.04);
    glEnd();
    glBegin(GL_POLYGON); //3rd part PATA
...
void Display(){
    ...
    ////////////////////////////moving boat
    glPushMatrix();
    glTranslatef(boat_move,0.0,0.0); //animation primitive
    Boat();
    glPopMatrix();

    boat_move += .005; //moving speed
    {
        if(boat_move > 1.9)
        {
            boat_move = -1.0;
    ...
    //////////////////////////moving cloud
    glPushMatrix();
    glTranslatef(cloud_move,0.0,0.0); //animation primitive
    Cloud1();
    glPopMatrix();

    glPushMatrix();
    glTranslatef(cloud_move1,0.0,0.0); //animation primitive
    Cloud2();
    glPopMatrix();

    glPushMatrix();
    glTranslatef(cloud_move2,0.0,0.0); //animation primitive
    Cloud3();
    glPopMatrix();

    cloud_move += .003; //moving speed
{
    if(cloud_move > 1.9)
    {
        cloud_move = -1.0;
...
cloud_move2 += .003; //moving speed
{
    ...
    ////////////////////////////////////sun move
    glPushMatrix();
    glTranslatef(0.0,sun_move,0.0); //animation primitive
    Sun();
    glPopMatrix();
...
if(start1==true)
{
    ...
    if(start2==true)//sun shine key
    {
        start3=false;
        glEnable(GL_LIGHT0);
        sun_move+=0.005; //moving speed
        if (sun_move>.30)
            {
            glEnable(GL_LIGHT0);
